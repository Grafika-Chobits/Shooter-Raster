...
Raster raster(int x1, int y1, int x2, int y2)

{

	Raster oke;

	oke.upleft 		= coord(x1,y1);

	oke.downright	= coord(x2,y2);
...
int isSingularAtas(Frame * frm, Coord posisi, RGB lineColor)

{

	int counter0 = 0;

	if(!isColorEqual(frm->px[posisi.x-1][posisi.y-1],lineColor))

		counter0++;

	if(!isColorEqual(frm->px[posisi.x][posisi.y-1],lineColor))
...
int isSingularBawah(Frame * frm, Coord posisi, RGB lineColor)
...
int isGarisDatar(Frame * frm, Coord posisi)

{

	if(isColorEqual(frm->px[posisi.x+1][posisi.y],frm->px[posisi.x][posisi.y])

		|| isColorEqual(frm->px[posisi.x-1][posisi.y],frm->px[posisi.x][posisi.y]))
...
int isSingular(Frame * frm, Coord posisi, RGB lineColor)

{

	if(isSingularAtas(frm, posisi, lineColor) || isSingularBawah(frm, posisi, lineColor))
...
void drawRaster(Frame * frm, Raster raster, RGB lineColor, RGB color) //

{
	...
	int i,j;
	 ...
	 for (j = raster.upleft.y; j <= raster.downright.y; j++)

	 {

	 	for(i = raster.upleft.x; i <= raster.downright.x; i++)

	 	{
...

	 		{
	 			...
	 			insertPixel(frm,coord(i,j),color);

	 		}

	 		

	 		if(isColorEqual(frm->px[i][j],lineColor) && (cangkul==0))

	 		{

	 			if(!isSingular(frm, coord(i,j), lineColor) && !isGarisDatar(frm, coord(i,j)))
	 		...
	 		else if(isColorEqual(frm->px[i+1][j],lineColor) && (cangkul==1))

	 		{

	 			if(!isSingular(frm, coord(i+1,j), lineColor) && !isGarisDatar(frm, coord(i+1,j)))
	 		...
	 		//else if(frm -> getColor(i,j).isEqual(color))
...
int main() {	
	...
	while (loop) {
		...
		plotLine(&canvas, 100, 50, 50, 100, rgb(255,244,99));

		plotLine(&canvas, 50, 100, 100, 150, rgb(255,244,99));

		plotLine(&canvas, 150, 100, 100, 150, rgb(255,244,99));
